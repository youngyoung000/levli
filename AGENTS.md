...
작업하는 AI 에이전트는 아래 순서를 그대로 따르면 된다. 판단이 필요한 지점은 §6에 모아 두었고,
그 항목은 임의로 결정하지 말고 사람에게 물어라.

- 라이브 확인: https://levli-landing.vercel.app
- 상태별 확인 링크: §5

**이 저장소의 화면은 디자인 확정본이다(2026-08-13 확정).** 임의로 레이아웃·간격·색을 바꾸지 말고,
필요하면 §6의 절차대로 사람에게 확인하라. Partners 로고 배치(카드 없이 46/33/36px·간격 60px)도
확정 사항이므로 실서비스의 흰 카드 방식으로 되돌리지 않는다.
...
## 6. 사람에게 확인해야 할 항목 — 임의로 결정하지 말 것

1. **`payout` → `reward` 용어 변경**이 컴플라이언스 검토 결과인지. 맞다면 Rules 본문과 ToS까지
   용어를 통일해야 한다.
2. **KYC 문구 완화** — "현재 Trial/Evaluation/Funded에 KYC를 요구하지 않는다"는 문장은 법무 검토
   대상이다. 확정 문구와 어긋나면 안 된다.
3. **법무 문서 링크**가 외부 도메인(`levli-trading.vercel.app`)을 가리킨다. 최종 도메인 확정 시 교체.
4. **홈 카피 중 미확정** — 실측 통계 노출 여부(현재는 사실 서술만 노출).

## 7. 문서

- `README.md` — 파일 맵·디자인 토큰·모션 스펙·데모 상태 재현법
